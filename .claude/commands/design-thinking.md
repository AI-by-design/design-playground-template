...
- A **locked problem statement** in the user's own words — not yours, not the product's.
- An **emotional core**: how the user feels before, during, and after the product. This is where brand comes from.
- A **brand identity** derived from that emotional core — argued, not picked from a mood board.
- All of it written into `CLAUDE.md`, so every future session (plan, build, evaluate) inherits the same brief.

Skip this and you get a generic-looking product that could have come from any AI. Run it and the output carries a specific point of view — because the agent knows exactly who it's building for and how it should feel.

Before asking the first question, say that upfront in your own words so the user knows what they're signing up for.
...
**Ask one question at a time.** Prefix each with `[Q x/6]` so the user always knows where they are in the phase. Wait for the answer before asking the next. If an answer drifts toward solutions, redirect before moving on: *"That's a solution — what's the underlying need?"* Once all six are answered, synthesize at the gate.
...
**Ask one question at a time.** Prefix each with `[Q x/5]`. Wait for the answer before asking the next. Push back on marketing-copy answers — the emotional core must be *felt* as true, not invented. Once all five are answered, synthesize at the gate.
...
**Ask one question at a time.** Prefix each with `[Q x/3]`. Wait for the answer before asking the next. Once all three are answered, derive the brand identity below.
