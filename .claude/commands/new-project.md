...
### 14. Offer to install Primitiv (optional, recommended)

After the dev server starts, ask the user:

```
Would you like to install Primitiv?

Primitiv is a design-contract layer that:
  - Adds a /build-component slash command (UI building against your contract)
  - Stops agents from hardcoding tokens or recreating components you already have
  - Works with Claude Code, Cursor, Codex, Windsurf — any MCP-compatible tool

Install now? (y / n / later)
```

**If `y`:** run `bunx @ai-by-design/primitiv init` in the project root. This is idempotent and does four things:
  - Writes a project-scoped `.mcp.json` with the Primitiv MCP server
  - Appends a `<!-- primitiv -->` block to `CLAUDE.md` telling agents how to use the contract
  - Installs `/build-component` at `.claude/commands/build-component.md` (project-level)
  - Adds Primitiv to `~/.codex/config.toml` if the `codex` CLI is on PATH

After init completes, tell the user: *"Primitiv installed. Run `bunx @ai-by-design/primitiv build` once you have components to scan — you can do that any time."* Do **not** run `primitiv build` automatically — there's nothing meaningful to scan yet (only shadcn defaults).

**If `n`:** skip. Make no changes.

**If `later`:** skip, and append this line to the `## Known Decisions` section of `CLAUDE.md`:
```
- Primitiv install deferred. To install: `bunx @ai-by-design/primitiv init`
```
...
After the dev server is running and the Primitiv question is resolved, print this exact block. Replace the `<primitiv-status>` placeholder based on the answer to step 14:
...
What just happened
- Next.js + TypeScript + Tailwind + shadcn/ui installed with Bun
- CLAUDE.md created (brief sections are empty — /design-thinking fills them)
- Cursor rules copied into .cursor/rules/
- Git initialised, GitHub repo created, Vercel linked
- Dev server running at http://localhost:3000
- <primitiv-status>

What's next — do NOT skip this
Run `/design-thinking` now. CLAUDE.md is a shell until it has the brief, and
every later command (/plan, /build-component, /evaluate) reads from it. If
you start coding without it, the agent will guess at the problem, the user,
and the brand — and you'll spend the next session undoing the guesses.

Shall I start /design-thinking now? (y/n)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

`<primitiv-status>` values:
- If Primitiv was installed: `Primitiv installed — /build-component available, run primitiv build after adding components`
- If skipped: `Primitiv skipped — install anytime with bunx @ai-by-design/primitiv init`
- If deferred: `Primitiv deferred — reminder added to CLAUDE.md`

If the user answers `y`, invoke `/design-thinking` immediately. If `n`, stop — do not begin any implementation work. No "just a quick feature first." The brief comes first.
